...
function template_body_above()
{
	...
	<div id="upper_container" class="wrapper"', empty($context['minmax_preferences']['upshrink']) ? '' : ' style="display: none;" aria-hidden="true"', '>
		<div id="upper_section">
			', call_template_callbacks('th', $context['theme_header_callbacks']), template_search_bar(), '
		</div>
		<div id="top_section">
			<div id="header">
				<div id="logobox">
...
function template_search_bar()
{
	...
	echo '
			<form id="search_form" action="', $scripturl, '?action=search;sa=results" method="post" accept-charset="UTF-8" autocomplete="off">
...
function template_menu()
{
	...
	{
		// Since some browsers don't support object-fit css, we need a container on the image so we can
		// make it a cover background on the container via JS
		if ($act === 'profile')
		{
			if (strpos($button['title'], '<img') !== false)
			{
				$button['title'] = preg_replace('~(<img.*?\/>).*?~', '<span id=\'menu_button_avatar\'>$1</span>$2', $button['title']);
			}
		}
...
function template_basicicons_legend()
{
	...
	echo '
		<p class="floatleft">', !empty($modSettings['enableParticipation']) && $context['user']['is_logged'] ? '
			<span class="topicicon i-participation"></span>' . $txt['participation_caption'] : '<span class="topicicon i-normal"> </span>' . $txt['normal_topic'], '<br />
